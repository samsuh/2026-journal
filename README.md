...
`FTs just change which space (real or recipricol) that the image is being represented in` but if we take only a sample from the FT, we get an image that resembles the original, we lose the details from the original image (lossy) 
