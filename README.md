...
`FTs just change which space (real or recipricol) that the image is being represented in` but if we take only a sample from the FT, we get an image that resembles the original, we lose the details from the original image (lossy); near the center of the ft is low-frequency oscillations, so keeping only the low frequency ones is a low pass filter. 
