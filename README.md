...
- eyedrops: 6:05am/ 3:20pm/ 11:20pm
- aspirin: 3:20pm
...
- slept again in the morning from like 630 to noonish. my sleeping schedule is so messed up. 
...
- eyedrops: around 1230am/ 10:25am/ 8pm
- aspirin: 8pm
- DONE ~~gym set B~~
...
- DONE ~~call KT about router~~
  ...
  - CLEAN UP!
    - did motorcycle laundry
    - cleared computer room floor mostly.
      - throw away old keyboard slider/ decide on throwing out old ipad case
      - clear desks of all stuff. put it in a doom pile in a diff room. sort.
      - put stuff in the bookshelf bins after sorted
...
- Math; i dont think i fundamentally understand working with matrices. i get how to do it, but im disconnected from first principles. I should pause the math stuff and go learn more about matrices, esp as used in linear algebra
  - came back around to 3b1b's linear algebra explainers. i should work through this before going back to applying it in code.
  - https://youtu.be/fNk_zzaMoSs?si=31SPnbNoytvtCKcZ
    - got through video 1: https://youtu.be/fNk_zzaMoSs?si=ieVb8eFnbVEQSrZ_
...
- before then finish packing.
  - PACKED ~~shirts from bedroom.~~
...
- Im so clumsy today. broke my keyboard tray and vacuum died while cleaning up. im a bit of a mess. 
...
- I want to get back to "being productive" sometime soon. I don't know what that means though. Im being pulled in a few directions; coding (threejs, rust, web dev, or ai stuff; both automation and practical), music (guitar, music production), aviation, gaming.
  - think in terms of workflow. what do you want to achieve? build a workflow to get you there that's fun and repeatable. discover your workflow
...
- Go to sleep early, at like 10pm. Wake up at 5 and get out by 6
...
- In progress. cleared up about 3-400 gb, but there's a lot more to do. ~~lets try clearing out old videos and free up space.~~ 
...
- DONE ~~Go for a ride to stretch back~~ nice ride to my old neighborhood in seoul forest and back. really enjoyable. 15km there and another 15 back.
  ...
  - luckily restarting the camera fixed something. at least the files are accessible. 
...
- Open mic went very well again. Everyone sounded good. Had a few small hiccups that got solved.
  - one weird thing is the one guy in the country i dislike showed up with their work friends, and idk how to feel abt that. ill just ignore them for the most part. 
...
## 3/16/26 - Monday 
Goals for today: 
- NOT DONE - didnt get to it. Focus back on creating something. review projects in progress and do a status update, or assess progress this year. 
- DONE ~~look up why i get bags under eyes. is it a health thing? coffee thing? im sleeping decently but its not going away. maybe an age thing?~~ dehydration and age
- DONE ~~Gym set B~~
...
- DONE ~~Made plans to goto lunch and goto Seoul Community Radio in the afternoon. Debating riding my bike up. It's drizzly today, so no on the bike idea.~~
  - SCR was fun. i should prob practice being social more cuz i caught myself blurting out things i prob shouldnt have said. oh well. glad i went out overall. 
...
- Finish haunted house?
  - Got through textures at least!
- DONE ~~laundry for big items~~
...
- consider adding multiple melodies, and give them their own space. maybe use different instruments so they dont clash. use different note ranges
- eq things to give space
- add bassline. 
...
Inversions can smoothen the movement between chords instead of jumping around octavely. Apparently its a rule for chords (in root position) that consecutive notes must be a third apart (major or minor) but cant be more/less than that. So if you keep widening a chord, eventually you break through back into a different chord. 

C (CEG) -> F (C->FA) -> G(->D->G->B) -> C (C<-E<--G<--)

E minor 1-6-2-5-1 progression 
root Emin (EGB) -> 6th Cmaj (EG->C) -> 2nd F#dim (-->F#->A C) -> 5th Bmaj(invert D# F#<--B<-) -> root Emin (->E->G B)
...
https://www.reddit.com/r/3Blue1Brown/comments/10nfx3r/is_there_a_fourier_transform_equivalent_for_3d/ 
https://en.wikipedia.org/wiki/Fourier_optics
Spectral Mesh Processing (paper: https://dl.acm.org/doi/epdf/10.1145/1837101.1837109) with implementation (https://github.com/LohitPetikam/SpectralMesh)
caltech video lessons on tranforms: https://youtu.be/fEyLh9HqsWU?si=4dwmZThL_SLvDHp4 watch these. 

> FTs just change which space (real or recipricol) that the image is being represented in 
- If we take only a sample from the FT of the low frequencies, we get an image that resembles the original, we lose the fine details from the original image; near the center of the ft is low-frequency oscillations, so keeping only the low frequency ones is a low pass filter.
- if we keep only the high frequency components (high pass filter), we get all the fine details, but not the basic info such as general darkness/lightness of the image.
- band-pass filter can pass mids only. 
...
- STARTED but not completed. ThreeJS - textures lesson
...
- DONE ~~Music making - recreate a single loop just to have done it. Stretch goal is to make an ABAC loop sequence.~~ https://youtu.be/RSnjD6xe5bM?si=zXtYdF9TjntddvA-
...
Listen to what youre making, and see what you like. 

"They need time. They need to get used to it. They're living it for the first ti-ime"
...
Another overview. https://youtu.be/zuZBkTNrv0g?si=bj7EffBtrRssc8IB
...
```
F - FACE - F Major 7
B - (FA)BD B minor 7 flat 5 inversion
...
- BDF - C+E+G+**BDF**, the B diminished 7 (all still in the key of C). 
- DFA - C+E+G+B+**DFA**, DFA is a 2nd interval, which is minor 2. so D minor. 
- FAC - C+E+G+B+D+**FAC** is an F Major, which is a 4th which is Major. F Major. 
- ACE - C+E+G+B+D+F+**ACE** is A minor, the 6th. A minor. 
- CEG - Back around to A+**CEG**, the octave note, which is Major. C Major. 
...
### Music production
Theres lots of chat about mastering. i dont really even know what that is yet. Looking into it today. 
...
The goal here is to approach playing chords in the scale, but to make it a little more musical by embellishing them; approaching the notes differently, arpeggiating them, sliding into them. Then using the adjacent chord to borrow notes from to give it some more movement. Change up the picking of the chord also works. 
...
He uses the first inversion chord (the 3rd is the lowest note) using the high 3 strings (thinnest 3); Root on high E string 3rd fret (G), fifth on 3rd fret of the B string (second thinnest), and third on the 4th fret of the G string (third thinnest). 

**First inversion G Major triad**
``` 
|-|-|R|-|
|-|-|5|-|
|-|-|-|3|
...
### Music Theory 
Im going to just spend some time beatmatching by ear. 
...
Trading: Getting rekt in the markets today. Caught a falling knife, and it keeps falling. haha. 

Practiced DJ stuff for a while. 

Learning music theory. question for today is that I want to understand how the chords in the scale work exactly. 

Question: How do the notes in the CDEFGAB C Major scale turn into I ii iii IV V chords in the C major scale? 
...
2. Learn to DJ to learn song structure and phrasing. Culmination: Perform a DJ set publicly somewhere in Seoul.

I've been learning that the little musical exposure I've had over the course of my life has not been directed properly, and that I really do want to have a creative outlet, and mastery of music as a skill set is something that I want to attain for myself. I know it takes years to master, but I'm confident I can get the basics down with a little focus and direction, and ideally, finding a method for me to pick it up is attainable as well. 
...
A summary of the 2025 one is that I lose a lot more time than I expect to when I go into autopilot mode. I can lose days or weeks at a time. I really think I have some form of ADHD, and the adhd term is "time blindness", which definitely seems to apply to me, along with a lot of habits that resonate pretty closely; most specifically, the hobby-collecting, dopamine-driven obsessive activity on certain topics. It's not all bad, and I like myself for the most part. 

One unintended consequence of the public journal is that some things that are a bit sensitive were released publically. It doesn't matter much because only one person reads this regularly (hi T), but i realized that not everything should be spoken about publicly, so some things will be held back due to some weird people being weird. 

Also, im going to write new stuff at the top so i dont have to scroll down. 
